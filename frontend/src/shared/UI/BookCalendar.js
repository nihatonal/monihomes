            ...
            <div className="calendar-modal-wrapper">
                <Calendar
                    ...
                    tileClassName={({ date, view }) => {
                        ...
                        if (selectedDays.slice(0, -1).filter((d) => check_out.includes(d)).find((x) => x === moment(date).format("YYYY/MM/DD"))

                        ) {
                            return "selected-check-in"
                        }

                        if (selectedDays.slice(1).filter((d) => check_in.includes(d)).find((x) => x === moment(date).format("YYYY/MM/DD"))
                        ...
                        if (check_in.find((x) => x === moment(date).format("YYYY/MM/DD"))) {
                            return "reserved_check-in"
                        }
                        if (check_out.find((x) => x === moment(date).format("YYYY/MM/DD"))) {
                        ...
                        // if (selected_dates.slice(1, -1).find((x) => x === moment(date).format("YYYY/MM/DD"))
                        //     && selected_dates.filter((d) => reserved.includes(d)).find((x) => x !== moment(date).format("YYYY/MM/DD"))
                        //     && !checker(reserved, selected_dates)
                        // ) {
                        //     return "selected";
                        // }
